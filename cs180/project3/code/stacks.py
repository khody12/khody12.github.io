...
def create_gaussian_stack(image, filter, stack, levels):
    if len(stack) == levels:
        return stack
    else:
        b, g, r = cv2.split(image.astype(np.float32))

        blurred_b = scipy.signal.convolve2d(b, filter, mode='same')
        blurred_g = scipy.signal.convolve2d(g, filter, mode='same')
...
def create_gaussian_stack_greyscale(image, filter, stack, levels):
    if len(stack) == levels:
        return stack
    else:
